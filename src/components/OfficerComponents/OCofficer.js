...
function OCofficer() {
    ...
    const fetchPolice = async () => {
        await axios
            .get(`/api/officervalueoc/${martic_number}/${unitId}`)
            .then((res) => {
    ...
    useEffect(() => {
        fetchPolice();
    }, [martic_number, unitId]);
    const Call = () => {
        axios(`/api/add-suspect-officeroc/${suspect_martic_number}`).then((res) => {
            if (res.status === 200) {
    ...
    const submitForm = (e) => {
        ...
        axios.put(`/api/officeroc/${police_id}`, data).then((res) => {
            ...
            if (res.data.status === 200) {
                if (location.pathname === `/unit1Osun/oc/add-suspect-officer/${martic_number}`) {
                    navigate.push(`/unit1Osun/oc`)
                            ...
                            <div className="row mt-4">
                                ...
                                <div className="col-md-12">
                                    ...
                                    <div className="row">
                                        ...
                                        <div>
                                            <h5>Note Taking On ({suspect_name}) By {item.iponote}</h5>
                                            <div className="container is-widescreen mb-4 " style={{ padding: '2rem', boxShadow: 'rgba(149, 157, 165, 0.2) 0px 8px 24px' }}>
                                                <p className="text-start">{item.note}</p>
                                            </div>
                                        </div>
    ...
    return (
        <>
            ...
            <Navbar />
            <div className="mt-5  m-5 container" id="oc-officer">
                            ...
                            >
                                <div>
                                    ...
                                    {police.map((item, index) => {
                                        return (
                                            <div key={index}>
                                                {item.oc_name?.length > 0 ?
                                                    <h2>Already Filled by OC POLICE OFFICER </h2> :
                                                    <>
                                                        <div className="col-md-12">
                                                            <h3
                                                                className="text-dark fs-5 mb-2"
                                                        ...
                                                        <div className="col-md-12">
                                                            ...
                                                            <textarea className="form-control" name="ocnote" value={input.ocnote} onChange={handleInput}></textarea>
                                                            <br />
